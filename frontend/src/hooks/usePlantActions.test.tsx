...
beforeEach(() => {
  ...
  vi.mocked(deleteUserPlant).mockResolvedValue(mockUserPlant);
  mockConfirm.mockReset();
});

describe('usePlantActions', () => {
  it('initialises editOpen and moveOpen to false', () => {
    const { result } = renderHook(() => usePlantActions(mockUserPlant), { wrapper: makeWrapper() });
  ...
  it('cloneMutation calls cloneUserPlant with the correct ids', async () => {
    ...
    await waitFor(() => expect(cloneUserPlant).toHaveBeenCalledWith('garden-1', 'bed-1', 'plant-1'));
  });
  ...
  it('handleDelete shows a confirm dialog with the plant name', async () => {
    ...
    expect(mockConfirm).toHaveBeenCalledWith(
      expect.objectContaining({ description: expect.stringContaining('Tomato') }),
    );
  });
  ...
  it('handleDelete includes the variety in the description when set', async () => {
    ...
    const { result } = renderHook(() => usePlantActions(plant), { wrapper: makeWrapper() });
    await act(async () => { await result.current.handleDelete(); });
    expect(mockConfirm).toHaveBeenCalledWith(
