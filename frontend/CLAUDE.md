...
- **`UserPlantDialog` SRP split** — the dialog currently has three responsibilities: dialog shell, edit form (owns `editForm`, plant catalog query, `updateMutation`, and renders fields inline), and create orchestration (owns garden/bed state + queries + `createMutation`). Split into: `UserPlantDialog` (shell only, ~30 lines), `UserPlantForm` (already exists, create mode), `UserPlantEditForm` (new, mirrors `UserPlantForm` for edit — owns form state, catalog query, update mutation).
