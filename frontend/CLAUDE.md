...
- **Plant icon on All Plants page** — the plant list items on `/plants` use a generic icon; they should display the same emoji/image as the bed canvas (via `plantEmoji()` / `plantImage()` from `src/lib/plants.ts`) so users can identify plants visually without navigating to the detail page.
- **Edit UI consistency** — the current mix of modals and slideouts is inconsistent and lacks a clear rule. Current state: All Gardens→edit→modal; Garden detail→edit garden→slideout; Garden detail→edit bed→modal; All Beds→edit bed→modal; Bed detail→edit bed→slideout; Bed detail→edit plant→slideout; All Plants→edit plant→modal; Plant detail→edit plant→slideout. The implied pattern (list pages = modal, detail pages = slideout) breaks down for secondary entity edits on detail pages (e.g. editing a bed from the garden page uses a modal, not a slideout). Before fixing, decide on a cohesive rule — one candidate: **modal for all edits triggered from a list or card context; slideout for all edits triggered from a detail page** — and apply it consistently across all entities.
...
- **`GardenGrid` SRP split** — component has grown to own state, multiple mutations, undo history, menu logic, inline render functions, dialogs, and orientation debouncing. Candidate splits: (1) `useGardenOrientation` hook — extract `localOrientation` state, debounce ref, `useEffect`, mutation, and `handleOrientationClick`; (2) `renderBedItem` / `renderFeatureItem` — extract inline render callbacks to standalone functions or small components; (3) `useGardenMenuItems` hook — extract `getMenuItems` + `handleDeleteItems`. Pure mechanical restructuring, no logic changes required.
