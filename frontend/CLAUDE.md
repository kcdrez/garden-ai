...
- **Edit UI consistency** — rule applied: modal for all edits triggered from a list or card context; slideout for all edits triggered from a detail page. Fixed: Garden detail canvas now uses `BedEditForm` (slideout) instead of `BedDialog` (modal) for the edit-bed action. All entities are now consistent.
