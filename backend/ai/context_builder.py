...
def _build_bed_context(bed: GardenBed) -> str:
    ...
    for up in plant_list:
        ...
        try:
            p = up.placement
            lines.append(f"Grid position: ({p.x}, {p.y}), {p.width} x {p.height} ft")
...
def _build_plant_context(user_plant: UserPlant) -> str:
    ...
    if placement:
        lines.append(f"Grid position: ({placement.x}, {placement.y}), {placement.width} x {placement.height} ft")
    else:
        lines.append("Not yet placed on grid")
