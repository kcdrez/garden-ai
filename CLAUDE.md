...
- **Canvas keyboard shortcuts (further)** — `?` to show a shortcut legend overlay (discoverability); `Ctrl+C`/`Ctrl+V` as a proper copy-paste pair (needs clipboard state + ghost placement indicator, not just an alias for Duplicate); keyboard entry into canvas without mouse (tabIndex on container, Tab selects first item when none selected)
