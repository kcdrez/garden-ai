...
- **Resize beds on garden layout** — `GardenGrid` doesn't pass `onResize` to `PlacementCanvas`, so beds have no resize handle; adding it requires a `resizeBedPlacement` API call (PATCH width/height on `BedPlacement`) and a new backend endpoint; note that `bedWidthFt`/`bedHeightFt` are currently computed from the bed's actual dimensions — resizing on the canvas would need to decouple placement size from bed size, or update the bed's dimensions directly
- **Bed canvas menu improvements** — the `...` menu on beds in `GardenGrid` currently has "Go to bed" and "Remove from layout"; add inline Edit (opens `BedDialog`) and Delete (with confirm) so common actions don't require navigating away
- **PlaceBedDialog "Create new bed" wizard** — `PlaceBedDialog` currently only lets the user pick from existing unplaced beds; add a wizard step (matching the "Add new plant" pattern in `PlacePlantDialog`) so a user can create a new bed and place it in one flow without leaving the canvas
...
- **Non-plant canvas objects** — allow placing decorative/structural items on both garden and bed canvases (e.g. trellis, pot, raised planter, path, fence, water feature, shed); useful for visualizing the full space — e.g. two beds with a trellis between them; design options: (a) a seeded global `GardenObject` catalog (similar to the plant catalog) with a `GardenObjectPlacement` model scoped to a garden or bed, or (b) freeform user-defined objects with just a label, shape, and dimensions; the `PlacementCanvas` `renderItem` callback already supports arbitrary SVG so the renderer change is minimal — the main work is the data model, catalog, and placement UI
- **Plant canvas "Remove from layout"** — `BedGrid` context menu has Edit, Move, Delete, Observations, Clone but no "Remove from layout" (delete `PlantPlacement` without deleting the `UserPlant`); the unplaced plants section already handles re-placing, so this is a natural companion action
- **Plant canvas hover tooltip** — show plant name + variety on hover so visually similar plants (e.g. Pie Pumpkin vs. Jack-o'-lantern Pumpkin) are distinguishable without opening the menu; a simple SVG `<title>` element inside the item group is the lowest-effort approach, but a styled HTML tooltip (positioned via `getBoundingClientRect` like the context menu) would be more readable
- **Persist canvas zoom to localStorage** — zoom currently resets to 0.75× on every page load; persist per bed/garden so the user's preferred zoom is remembered; `PlacementCanvas` would need an optional `storageKey` prop (e.g. `canvas-zoom-bed-{bedId}`, `canvas-zoom-garden-{gardenId}`); `BedGrid` and `GardenGrid` pass the key, `PlacementCanvas` reads/writes via `localStorage` instead of plain `useState`
- **Keyboard shortcuts** — `Ctrl+C/V/X` for copy/paste/cut of placed items; `Delete`/`Backspace` to remove selected item; arrow keys to nudge position by small increments
- **Multi-select** — click + drag to rubber-band select multiple items, or `Shift+click` to add to selection; move the group together; scale all selected items proportionally; clone the whole selection at once
