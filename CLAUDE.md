...
- North orientation + item rotation — `Garden.orientation` (0–315°, 45° steps); compass rose pill above garden canvas (sticky, centered, debounced click); `rotation` field on all three placement types; SVG rotate transform on canvas items; `[`/`]` shortcuts; sticky navbar; `--spacing-navbar` CSS token
- Bed orientation — replaced `GardenBed.facing` (string N/NE/E/SE/S/SW/W/NW) with `GardenBed.orientation` (same numeric 0–315° format as `Garden.orientation`); data migration converts existing values; compass rose on bed canvas (debounced, same UX as garden); "Top of bed faces" in create form only; orientation removed from edit form and bed details (canvas is the canonical control); AI context builder updated
