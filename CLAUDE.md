...
- **Canvas keyboard shortcuts (further)** — `Ctrl+C`/`Ctrl+V` copy-paste (needs clipboard state + ghost placement indicator); keyboard entry into canvas without mouse (tabIndex on container, Tab selects first item when none selected)
